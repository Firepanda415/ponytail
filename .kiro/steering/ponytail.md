...
Bug fix = root cause, not symptom: a report names a symptom. Grep every caller of the function you touch. Fix the representation, operation or lifecycle rule at the first owner that violates the intended relation, then check every affected caller. Keep the correction at that owner instead of patching the same symptom separately in each caller.
